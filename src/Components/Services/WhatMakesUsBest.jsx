...
function WhatMakesUsBest() {
  ...
  return (
      <div className="h-[70vh] flex bg-[linear-gradient(93deg,_#3270BC_0%,_#092B60_48.48%,_#181325_97.53%)] backdrop-blur-[3px] mt-[35%] p-24">
        <div className="w-1/3 text-[#FFFFFF] text-center ">
          <h1 className="font-light text-6xl">What Makes Us</h1>
          <h1 className="font-semibold text-8xl mt-2 mb-8">The Best</h1>
          <Link
              to="/contact"
              className="group relative transition-all ml-16 duration-300 text-[#000000] text-lg px-6 py-3 rounded-full bg-[#FFFFFF] font-medium flex items-center justify-between w-[65%]"
            >
              Connect With Us
        ...
        <div className="grid grid-cols-2 gap-6 mx-16">
          {services.slice(3, 5).map((service, index) => (
            <div key={index} className="bg-[#FFFFFF1A] p-7 rounded-lg backdrop-blur-md flex flex-col items-center text-center">
              <div className="text-3xl text-[#41BEA1] mb-2 w-8 h-8">{service.icon}</div>
              <p className="text-base text-[#FFFFFF]">{service.text}</p>
            </div>
          ))}
